...
describe('EmployeeListComponent', () => {
  ...
  let hrApiSpy: jasmine.SpyObj<HrApiService>;
  ...
  beforeEach(async () => {
    ...
    hrApiSpy = jasmine.createSpyObj('HrApiService', ['getEmployees', 'createEmployee', 'updateEmployee', 'deleteEmployee']);
    hrApiSpy.getEmployees.and.resolveTo(MOCK_EMPLOYEES.map(e => ({ ...e })));
    hrApiSpy.createEmployee.and.resolveTo(undefined as never);
    hrApiSpy.updateEmployee.and.resolveTo(undefined);
    hrApiSpy.deleteEmployee.and.resolveTo(undefined);
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: HrApiService, useValue: hrApiSpy }
      ]
    }).compileComponents();
  ...
  it('should load employees from the API on initialization', async () => {
    expect(hrApiSpy.getEmployees).toHaveBeenCalled();
  ...
  it('saveEmployee() in add mode should call the API and show toast', async () => {
    ...
    expect(hrApiSpy.createEmployee).toHaveBeenCalled();
    expect(toastSpy.success).toHaveBeenCalledWith('New employee registered successfully.');
    expect(component.showModal()).toBeFalse();
  });

  it('saveEmployee() in edit mode should call the update API and show toast', async () => {
    const emp = MOCK_EMPLOYEES[0];
    component.isEditMode = true;
    component.currentEmp = { ...emp, position: 'Updated Position' };
    await component.saveEmployee();
    expect(hrApiSpy.updateEmployee).toHaveBeenCalled();
    expect(toastSpy.success).toHaveBeenCalledWith('Employee profile updated successfully.');
  });

  it('deleteEmployee() should not delete when dialog returns false', async () => {
    ...
    expect(hrApiSpy.deleteEmployee).not.toHaveBeenCalled();
    expect(component.employees().length).toBe(initialCount);
  });

  it('deleteEmployee() should delete employee via API and show toast when dialog returns true', async () => {
    dialogSpy.confirm.and.returnValue(Promise.resolve(true));
    const empId = component.employees()[0].id;
    await component.deleteEmployee(empId);
    expect(hrApiSpy.deleteEmployee).toHaveBeenCalledWith(empId);
    expect(toastSpy.success).toHaveBeenCalledWith('Employee record deleted.');
  });

  it('deleteEmployee() should call dialog.confirm with danger type', async () => {
    dialogSpy.confirm.and.returnValue(Promise.resolve(false));
